...
const ws = fs.createWriteStream('./probable-data.csv');
const stream = csv.format();
stream.pipe(ws);

//make a header row for csv//
stream.write(['team', 'probablePitchers']);
//these lines determine which dates to scrape//
const today = new Date();
const today_date = new Date(today.setDate(today.getDate()));
...
//   if (game_status !== "Final" && game_type === "R") { return }
  ...
  const gamepk = data.gameData.game.pk;
    ...
    // playThree.away_Score = fb.result.awayScore;
        ...
        // TOP FIFTH
        const playProb = {}
        const pp_date = data.gameData.datetime.officialDate;
        if (pp_date === '2024-04-03') {
            let pp_home_away = ['home', 'away'];
            let probablePitchers = []; // Array to store probable pitchers for both teams
            let teams = []; // Array to store team abbreviations for both teams
        
            for (let i = 0; i < pp_home_away.length; i++) {
                const pp = data.gameData.probablePitchers[pp_home_away[i]].fullName;
                const team = data.gameData.teams[pp_home_away[i]].abbreviation;
        
                probablePitchers.push(pp); // Store probable pitcher for the current team
                teams.push(team); // Store team abbreviation for the current team
            }
        
            // Now you have both probable pitchers and team abbreviations for both 'home' and 'away' teams
            console.log('Probable pitchers for home team:', probablePitchers[0]);
            console.log('Team abbreviation for home team:', teams[0]);
            console.log('Probable pitchers for away team:', probablePitchers[1]);
            console.log('Team abbreviation for away team:', teams[1]);
        
            // If you want to store these values in a single object:
            const playProb = {
                home: {
                    probablePitcher: probablePitchers[0],
                    teamAbbreviation: teams[0]
                },
                away: {
                    probablePitcher: probablePitchers[1],
                    teamAbbreviation: teams[1]
                }
            };
        
            console.log(playProb); // Output the playProb object
        }

        // const fb = data.liveData.plays.allPlays[topFifth];
        // TOP FIRST: 
        // playProb.gamepk = gamepk;
        // playProb.date = data.gameData.datetime.officialDate;
        // playProb.pitcher = fb.matchup.pitcher.id;
        // playProb.pitcher_name = fb.matchup.pitcher.fullName;
        // playProb.throws = fb.matchup.pitchHand.code;
        // playProb.events = fb.result.event;
        // playProb.des = fb.result.description;
        // playProb.inning = fb.about.inning;
        // playProb.topbot = fb.about.halfInning;
        // const pbteamAbbrv = playProb.topbot == 'top' ? data.gameData.teams.home.abbreviation : data.gameData.teams.away.abbreviation;
        // playProb.teamAbbrv = pbteamAbbrv;
        // if(playProb.pitcher_name == play.pitcher_name){
        //     const finishedFive = 1
        //     playProb.finished5 = finishedFive;
        //     playProb.pitcher_name = fb.matchup.pitcher.fullName;
        // } else {
        //     const finishedFive = 0
        //     playProb.finished5 = finishedFive;
        //     playProb.pitcher_name = ab.matchup.pitcher.fullName;
        // }
        // if(playFour.home_Score > playFour.away_Score){
        //     const won = 1;
        //     const loss = 0;
